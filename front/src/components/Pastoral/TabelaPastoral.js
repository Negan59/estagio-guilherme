...
const TabelaPastorais = () => {
  ...
  const buscarPastorais = async () => {
    try {
      console.log(`http://localhost:8080/api/pastoral/${abaAtiva}`)
      const response = await axios.get(`http://localhost:8080/api/pastoral/${abaAtiva}`);
  ...
  const deletaPastoral = async (id) => {
    try {
      if (window.confirm('Tem certeza que deseja excluir esta pastoral?')) {
        await axios.delete(`http://localhost:8080/api/pastoral/${abaAtiva}/${id}`);
