...
const ModalPastoral = ({ pastoral, onSubmit, onClose }) => {
  ...
  const buscarParoquianoPorId = async (paroquianoId) => {
    try {
      const response = await axios.get(`http://localhost:8080/api/paroquiano/${paroquianoId}`);
  ...
  const handleSubmit = async (event) => {
    ...
    if (id) {
      ...
      try {
        const response = await axios.put('http://localhost:8080/api/pastoral', data);
      ...
      try {
        const response = await axios.post('http://localhost:8080/api/pastoral', data);
